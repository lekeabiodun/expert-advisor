...
void OnTick() 
{
    ...
    // int mxPrice = MathMax( (int)  ((maxPrice - ((int) maxPrice) ) * 1000), (int) ((minPrice - ((int) minPrice) ) * 1000) );
    // int mnPrice = ;

    Print("Max: ", maxPrice, " Min: ", minPrice);

    for(double i = mnPrice; i <= mxPrice; i++) 
    {
        // double cp = (i - (int) i) * 1000;
        Print(i, " % ", levelModulo, " = ", MathMod(i, levelModulo));
