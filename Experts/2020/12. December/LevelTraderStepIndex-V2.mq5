...
void OnTick() 
{
    ...
    // if(OrdersTotal() + PositionsTotal() >= 4) { return; }
    ...
    if(!OrdersTotal() && !PositionsTotal())
    {
        ...
        // trade.SellStop(lotSize + 3, NextLevel-7.5, Symbol(), 0, 0);

        trade.SellStop(lotSize, PrevLevel, Symbol(), NextLevel, PrevLevel-levelModulo);
        // trade.BuyStop(lotSize + 3, NextLevel+7.5, Symbol(), 0, 0);
        ...
        //         // trade.SellStop(lotSize + 3, NextLevel-, Symbol(), 0, 0);
...
void TakeCoverTrade(double CurrPrice, ENUM_DEAL_TYPE DealType) 
{
    ...
    if(DealType == DEAL_TYPE_BUY)
    {
        ...
        trade.SellStop(CoverLotSize, PrevLevel, Symbol(), CurrLevel, PrevLevel-(levelModulo - CoverLevel));
    ...
    if(DealType == DEAL_TYPE_SELL)
    {
        ...
        trade.BuyStop(CoverLotSize, NextLevel, Symbol(), CurrLevel, NextLevel+(levelModulo - CoverLevel));
    }
