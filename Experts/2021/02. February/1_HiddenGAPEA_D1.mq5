...
void TakeTrade(ENUM_MARKET_ENTRY Entry) 
{
    ...
    if(Entry == MARKET_ENTRY_LONG && ExpertIsTakingBuyTrade) {
        ...
        trade.BuyStop(LotSize, NormalizeDouble(entry_price, _Digits), Symbol(), sl, tp);
    }
    ...
    if(Entry == MARKET_ENTRY_SHORT && ExpertIsTakingSellTrade) {
        ...
        // Print("available_point: ", available_point);
        // Print("available_pip: ", available_pip);
        // Print("entry_price: ", entry_price);
        // Print("gap_percentile_entry: ", gap_percentile_entry);
        // Print("gap_percentile_exit: ", gap_percentile_exit);
        // Print(":GP: ", GapPercentileEntry);
        // Print(":GP: ",  GapPercentileEntry / 100.0);
        // Print(":GP: ",  available_pip * (GapPercentileEntry / 100));
        trade.SetExpertMagicNumber(EXPERT_MAGIC);
        trade.SellStop(LotSize, NormalizeDouble(entry_price, _Digits), Symbol(), sl, tp);
    }
