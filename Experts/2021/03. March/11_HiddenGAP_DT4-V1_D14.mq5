...
void OnTick() 
{
    ...
    if(iOpen(Symbol(), ExpertHigherTimeFrame, 0) > GapPrice)
    {
        ...
        if(DSS(MARKET_SIGNAL_SELL) && FindNewGAP() && (bid > EntryPrice))
        {
    ...
    if(iOpen(Symbol(), ExpertHigherTimeFrame, 0) < GapPrice)
    {
        ...
        if(DSS(MARKET_SIGNAL_BUY) && FindNewGAP() && (ask < EntryPrice))
        {
...
bool DSS(ENUM_MARKET_SIGNAL Signal) 
{
    ...
    if(Signal == MARKET_SIGNAL_BUY && DSSValue0 > DSSOverSoldLevel && DSSValue1 < DSSOverSoldLevel && UnlockBUY) 
    { 
        UnlockSELL = true;
        UnlockBUY = false;
    ...
    if(Signal == MARKET_SIGNAL_SELL && DSSValue0 < DSSOverBoughtLevel && DSSValue1 > DSSOverBoughtLevel && UnlockSELL) 
    { 
        UnlockBUY = true;
        UnlockSELL = false;
...
void SetTrade(ENUM_MARKET_ENTRY Entry) 
{
    ...
    if(Entry == MARKET_ENTRY_LONG && ExpertIsTakingBuyTrade) 
    {
        double open                             = iOpen(Symbol(), ExpertHigherTimeFrame, 0);
        EntryPrice                              = NormalizeDouble(open + NormalizeDouble(EntryPadding / MathPow(10, _Digits), _Digits), _Digits);
        StopLoss                                = NormalizeDouble(EntryPrice - (InputStopLoss / MathPow(10, _Digits)), _Digits);
        TakeProfit                              = NormalizeDouble(EntryPrice + (InputTakeProfit / MathPow(10, _Digits)), _Digits);
    }
