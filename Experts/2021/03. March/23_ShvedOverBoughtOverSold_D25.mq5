...
input int                                           InputMaxSpread = 1; // Max Spread to trade
...
void OnTick() 
{
    ...
    if(new_zone == 5 && PrevTradeZoneHigh != SupportZoneHigh && PrevTradeZoneLow != SupportZoneLow && WPR(MARKET_ENTRY_LONG) && CandleShape(MARKET_ENTRY_LONG))
    {
    ...
    if(new_zone == 4 && PrevTradeZoneHigh != ResistantZoneHigh && PrevTradeZoneLow != ResistantZoneLow && WPR(MARKET_ENTRY_SHORT) && CandleShape(MARKET_ENTRY_SHORT))
    {
...
bool WPR(ENUM_MARKET_ENTRY Signal)
{
    double WPRArray[];
    ArraySetAsSeries(WPRArray, true);
    CopyBuffer(WPRHandle, 0, 0, 2, WPRArray);

    if(Signal == MARKET_ENTRY_LONG && WPRArray[0] < WPROverSoldLevel) 
    { 
        return true; 
    }
    if(Signal == MARKET_ENTRY_SHORT && WPRArray[0] > WPROverBoughtLevel) 
    { 
        return true; 
    }
...
//+------------------------------------------------------------------+
//| CandleShape function                                             |
//+------------------------------------------------------------------+
input group                                         "============  CandleShape Settings ===============";
input bool                                          UseCandleShape = false; // Use CandleShape Filter
input ENUM_TIMEFRAMES                               FilterTimeframe = PERIOD_D1; // CandleShape Timeframes

bool CandleShape(ENUM_MARKET_ENTRY Signal)
{
    if(!UseCandleShape) { return true; }
    
    if(Signal == MARKET_ENTRY_LONG && iClose(Symbol(), FilterTimeframe, 0) > iOpen(Symbol(), FilterTimeframe, 0)) 
    { 
        return true; 
    }

    if(Signal == MARKET_ENTRY_SHORT  && iClose(Symbol(), FilterTimeframe, 0) < iOpen(Symbol(), FilterTimeframe, 0)) 
    {
        return true; 
    }
