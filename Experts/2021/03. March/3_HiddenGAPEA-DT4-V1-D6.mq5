...
input double                                        SwingPadding = 100; // Swing Padding
input double                                        SwingDefault = 200; // Swing Default
input double                                        TakeProfit = 100; // Take Profit in Pips
input double                                        EntryPadding = 10; // Entry Padding in Pips
input ENUM_TIMEFRAMES                               ExpertStopLossTimeFrame = PERIOD_CURRENT; // StopLoss Timeframe
...
void TakeTrade(ENUM_MARKET_ENTRY Entry) 
{
    ...
    if(Entry == MARKET_ENTRY_LONG && ExpertIsTakingBuyTrade) 
    {
        ...
        double sl			                    = CalculateStopLoss(MARKET_ENTRY_LONG) - NormalizeDouble(SwingPadding / MathPow(10, _Digits), _Digits);
        if((entry_price - sl) > NormalizeDouble(SwingDefault / MathPow(10, _Digits), _Digits)) { sl = entry_price - NormalizeDouble(SwingDefault / MathPow(10, _Digits), _Digits); }
    ...
    if(Entry == MARKET_ENTRY_SHORT && ExpertIsTakingSellTrade) 
    {
        double open                             = iOpen(Symbol(), Period(), 0);
        double entry_price                      = NormalizeDouble(open - NormalizeDouble(EntryPadding / MathPow(10, _Digits), _Digits), _Digits);
        double sl			                    = CalculateStopLoss(MARKET_ENTRY_SHORT) + NormalizeDouble(SwingPadding / MathPow(10, _Digits), _Digits);
        if((sl - entry_price) > NormalizeDouble(SwingDefault / MathPow(10, _Digits), _Digits)) { sl = entry_price + NormalizeDouble(SwingDefault / MathPow(10, _Digits), _Digits); }
        double tp                               = NormalizeDouble(entry_price - (TakeProfit / MathPow(10, _Digits)), _Digits);
        trade.SetExpertMagicNumber(EXPERT_MAGIC);
...
double CalculateStopLoss(ENUM_MARKET_ENTRY Entry)
{
    double StopLoss;

    if(Entry == MARKET_ENTRY_LONG) 
    {
        StopLoss = iLow(Symbol(), ExpertStopLossTimeFrame, 0);

        for(int i = 0; i < SwingPeriod; i++ )
        {
            StopLoss = MathMin(StopLoss, iLow(Symbol(), ExpertStopLossTimeFrame, i));
        }
    ...
    {
        StopLoss = iHigh(Symbol(), ExpertStopLossTimeFrame, 0);

        for(int i = 0; i < SwingPeriod; i++ )
        {
            StopLoss = MathMax(StopLoss, iHigh(Symbol(), ExpertStopLossTimeFrame, i));
        }
