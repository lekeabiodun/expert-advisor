...
{
    if(Signal == MARKET_ENTRY_SHORT && iClose(Symbol(), Period(), 1) < iOpen(Symbol(), Period(), 1))
    {
        return true;
    }

    if(Signal == MARKET_ENTRY_LONG && iClose(Symbol(), Period(), 1) > iOpen(Symbol(), Period(), 1))
    {
        return true;
    }
...
double GetLotSize() 
{
    ...
    double dealLostPerTrade = InputLotSize * NormalizeDouble(InputStopLoss / MathPow(10, _Digits), _Digits);

    if(dealLost <= dealLostPerTrade) { return InputLotSize; }

    RecoveryLotSize = NormalizeDouble((dealLost * (InputTakeProfit / InputStopLoss)) / InputTakeProfit, 2);
