...
input string                                        tradingHourToSkipString = "1,2,3,4,5,6,7,8,9"; // Trade Hour to  skip  
...
bool TradingPeriod()
{
    ...
    StringSplit(tradingHourToSkipString,separator,tradingHourToSkipArray);
    ...
    for(int i = 0; i < ArraySize(tradingHourToSkipArray); i++) { 
        if(StringCompare((string) tradePeriodCurrentTime.hour, tradingHourToSkipArray[i]) == 0) {return false; }
    }
