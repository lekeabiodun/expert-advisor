...
void OnTick() 
{
    ...
    if(moving_average_signal(SELL) && MFISignal) {
        for(int i = 0; i < 100; i++)
        {
            if(iClose(Symbol(), Period(), i) > iOpen(Symbol(), Period(), i))
            {
                stopLoss = iHigh(Symbol(), Period(), i) - iClose(Symbol(), Period(), 0) + stopL;
                break;
            }
        }
        Comment("Trade taken");
...
bool moving_average_signal(marketSignal signal){
    ...
    else if(signal == SELL && FastMovingAverageArray[0] < SlowMovingAverageArray[0] && !MASELL) 
    { 
        MABUY = false;
        MASELL = true;
        return true; 
