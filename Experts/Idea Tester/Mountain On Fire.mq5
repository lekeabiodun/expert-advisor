...
input int                                           candlePeriod = 100; 
input int                                           candleDiff = 100;
...
void OnTick() 
{
    ...
    if(kalman_filter_signal(BUY) ) { 
        takeTrade(LONG);
    }

    if(kalman_filter_signal(SELL) && candle_filter()) { 
        takeTrade(SHORT);
    }

}
bool candle_filter(){
    double priceLow = iLow(Symbol(), Period(), 0);
    double priceHigh = iHigh(Symbol(), Period(), 0);
    for(int i = 0; i < candlePeriod; i++) {
        priceLow = MathMin(priceLow, iLow(Symbol(), Period(), i));
        priceHigh = MathMax(priceHigh, iHigh(Symbol(), Period(), i));
    }
    // Print("Price low: ", priceLow, " price high: ", priceHigh, " = ",  priceHigh - priceLow);
    // stopLoss = priceHigh - iClose(Symbol(), Period(),0);
    return (priceHigh - priceLow) >= candleDiff;
}
...
bool kalman_filter_signal(marketSignal signal){
    ...
    CopyBuffer(kfHandle, 1, 1, 2, kfArray);
    // Print(kfArray[0]);
