...
input int                                           tradeRange = 50; // Trade range in Pips
...
void OnTick() 
{
    ...
    // ObjectCreate( 0, "upperLine", OBJ_HLINE, 0, 0, DoubleToString(price + tradeRange));
    // ObjectCreate( 0, "middleLine", OBJ_HLINE, 0, 0, DoubleToString(price));
    // ObjectCreate( 0, "lowerLine", OBJ_HLINE, 0, 0, DoubleToString(price - tradeRange));

    if(iClose(Symbol(), Period(), 1) > iOpen(Symbol(), Period(), 1)) {
        prev_price = iLow(Symbol(), Period(), 1);
        curr_price = iHigh(Symbol(), Period(),0);
    } else {
        prev_price = iHigh(Symbol(), Period(),1);
        curr_price = iLow(Symbol(), Period(),0);
    }

    min_price = MathMin(prev_price, curr_price);
    max_price = MathMax(prev_price, curr_price);

    for(int i = (int) min_price; i <= max_price; i++) {

        if((i % tradeRange) == 0) {

            if(prev_price > i && curr_price < i && !MABUY) {
                MABUY = true;
                MASELL = false;
                takeTrade(SHORT); 
            }

            if(prev_price < i && curr_price > i && !MASELL) {
                MABUY = false;
                MASELL = true;
                takeTrade(LONG);
             }
