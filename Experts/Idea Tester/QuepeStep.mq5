...
void OnTick() 
{
    ...
    double price = MathRound(iClose(Symbol(), Period(), 0) / tradeRange) * tradeRange;
    ...
    double prev_price = iOpen(Symbol(), Period(), 1);
    double curr_price = iOpen(Symbol(), Period(), 0);
    double tradePrice = MathRound( ( (prev_price + curr_price) / 2 ) / tradeRange ) * tradeRange;

    if(prev_price < tradePrice && curr_price > tradePrice && !MABUY) {
        MABUY = true;
        MASELL = false;
        takeTrade(SHORT); 
    }

    if(prev_price > tradePrice && curr_price < tradePrice && !MASELL) {
        MABUY = false;
        MASELL = true;
        takeTrade(LONG);
    }
...
void takeTrade(marketEntry entry) {  
   ...
   if(entry == LONG && expertIsTakingBuyTrade) {
        ...
        trade.Buy(lotSize, Symbol(), ask, ask - uptrendStopLoss, ask + uptrendTakeProfit);
   }
   if(entry == SHORT && expertIsTakingSellTrade) {
        double bid = NormalizeDouble(SymbolInfoDouble(Symbol(), SYMBOL_BID), _Digits);
        trade.SetExpertMagicNumber(EXPERT_MAGIC);
        trade.Sell(lotSize, Symbol(), bid, bid + downtrendStopLoss, bid - downtrendTakeProfit);
   }
}
