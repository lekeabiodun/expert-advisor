...
input int                                           tradeTimeTarget = 60; // Trade Time Target
...
void takeTrade(marketEntry entry) {      
   ...
   if(entry == LONG && expertIsTakingBuyTrade) {
        ...
        if(tradeOppositePendingOrder){
            trade.SellStop(lotSize, ask-pendingOrderAwayPips, Symbol(), 0, 0, ORDER_TIME_GTC,0);
        }
   ...
   if(entry == SHORT && expertIsTakingSellTrade) {
        ...
        if(tradeOppositePendingOrder){
            trade.BuyStop(lotSize, bid+pendingOrderAwayPips, Symbol(), 0, 0, ORDER_TIME_GTC,0);
       }
        if(getPreviousDealLost() > recoveryCount && expertIsTakingRecoveryTrade){
...
void tradePositionManager() {
    for(int i = PositionsTotal()-1; i >= 0; i--) {
        PositionGetSymbol(i);
        if((int)TimeCurrent() - PositionGetInteger(POSITION_TIME) >= tradeTimeTarget) {
            ulong ticket = PositionGetTicket(i);
            trade.PositionClose(ticket);
            if(OrdersTotal()) {
...
bool etv_signal(marketSignal signal)
{
    ...
    if(signal == BUY && ETVBuy[0] != EMPTY_VALUE && ETVEnd[0] == EMPTY_VALUE && !MABUY) {
        if(MASELL && closeOppositeTradeOnOppositeSignal) { close_all_positions(); }
        MABUY = true;
        MASELL = false;
        return true;
    }
    if(signal == SELL && ETVSell[0] != EMPTY_VALUE && ETVEnd[0] == EMPTY_VALUE && !MASELL) {
        if(MABUY && closeOppositeTradeOnOppositeSignal) { close_all_positions(); }
        MABUY = false;
        MASELL = true;
        return true; 
    }
    if(ETVEnd[0] != EMPTY_VALUE ) {
        MABUY = false;
        MASELL = false;
    }
   return false;
