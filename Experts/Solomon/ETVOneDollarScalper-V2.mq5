...
input double                                        pendingOrderTakeProfit = 1; // Pending Order Take Profit in Pips
input double                                        pendingOrderStopLoss = 70; // Pending Order Stop Loss in Pips
...
double calculatedDowntrendTakeProfit; 
double calculatedDowntrendStopLoss; 
...
void takeTrade(marketEntry entry) {      
   ...
   if(entry == LONG && expertIsTakingBuyTrade) {
        ...
        pendingOrderPrice = etvLineValue - pendingOrderAwayPips;
   ...
   if(entry == SHORT && expertIsTakingSellTrade) {
        ...
        calculatedDowntrendStopLoss = downtrendStopLoss;
        calculatedDowntrendTakeProfit = downtrendTakeProfit;
...
void tradePendingOrder()
{
    ...
    if(pendingOrderEntry == LONG && pendingOrderPrice) {
        double bid = NormalizeDouble(SymbolInfoDouble(Symbol(), SYMBOL_BID), _Digits);
        calculatedDowntrendStopLoss = pendingOrderStopLoss;
        calculatedDowntrendTakeProfit = pendingOrderTakeProfit;
...
void tradePositionManager() {
    for(int i = PositionsTotal()-1; i >= 0; i--) {
        ...
        if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL) {
            if(PositionGetDouble(POSITION_PROFIT) >= (lotSize * calculatedDowntrendTakeProfit)) {
                ulong ticket = PositionGetTicket(i);
                trade.PositionClose(ticket);
            }
            if(PositionGetDouble(POSITION_PROFIT) <= -(lotSize * calculatedDowntrendStopLoss)) {
                ulong ticket = PositionGetTicket(i);
                trade.PositionClose(ticket);
            }
        }
