...
input bool                                          expertIsTradingSwing = false; // Trade Swing
...
void takeTrade(marketEntry entry) {      
   if(entry == LONG && expertIsTakingBuyTrade) {
        ...
        trade.Buy(lotSize, Symbol(), ask, ask - uptrendStopLoss, 0);
        if(getPreviousDealLost() > recoveryCount  && expertIsTakingRecoveryTrade){
            trade.Buy(recoveryLotSize, Symbol(), ask, ask - uptrendStopLoss, 0, "Recovery");
        }
   }
   if(entry == SHORT && expertIsTakingSellTrade) {
        double bid = NormalizeDouble(SymbolInfoDouble(Symbol(), SYMBOL_BID), _Digits);
        double takeProfitPrice = bid - downtrendTakeProfit;
        if(expertIsTradingSwing) {
            takeProfitPrice = bid - ((iHigh(Symbol(), Period(), 1) - iLow(Symbol(), Period(), 1) ) * swingConstant );
        }
        trade.SetExpertMagicNumber(EXPERT_MAGIC);
        trade.Sell(lotSize, Symbol(), bid, 0, takeProfitPrice);
        if(getPreviousDealLost() > recoveryCount && expertIsTakingRecoveryTrade){
            trade.Sell(recoveryLotSize, Symbol(), bid, 0, takeProfitPrice, "Recovery");
        }
   }
}
...
bool spike_signal(marketSignal signal ) {
   if(iHigh(Symbol(), Period(), 1) > iOpen(Symbol(), Period(), 1)) { 
       if(signal == BUY && useBuyTradeMomentum) {
           for(int i=2; i<1440; i++) {
               if(iOpen(Symbol(), Period(), i) >= iHigh(Symbol(), Period(), i)) {
                   continue;
               }
               if(iHigh(Symbol(), Period(), i) > iOpen(Symbol(), Period(), i)) {
                   if(iHigh(Symbol(), Period(), 1) > iHigh(Symbol(), Period(), i)) {
                       return true;
                   } else {
                       return false;
                   }
