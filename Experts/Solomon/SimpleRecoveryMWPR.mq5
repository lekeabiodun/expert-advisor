...
input double                                       wprSmoothingPeriod = 0; // WPR Smoothing Period
...
int wprHandle = iCustom(NULL, 0, "mwpr", wprPeriod, wprSmoothingPeriod);
...
bool wpr_signal(marketSignal signal){
   ...
   if(signal == BUY && ( wprValue > wprOverBoughtLevel ) && !wprOverBoughtSignalCrossUp)
    {
        Print("Cross Overbought up: ", wprValue);
    ...
    {
        Print("Cross Oversold up: ", wprValue);
    ...
    {
        Print("Cross Overbought down: ", wprValue);
