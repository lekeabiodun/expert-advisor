...
void takeRecoveryTrade(marketEntry entry) {
    if(entry == LONG && expertIsTakingBuyTrade) {
        ...
        if( ( (dealLost/recoveryPips) / (-1) ) < smallestLotSize ) { recoveryLotSize = smallestLotSize; }
        else if( ( (dealLost/recoveryPips) / (-1) ) > biggestLotSize ) { recoveryLotSize = biggestLotSize; }
        else { recoveryLotSize = NormalizeDouble(( (dealLost/recoveryPips) / (-1) ), 3); }
