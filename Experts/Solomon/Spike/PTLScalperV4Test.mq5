...
void takeTrade(marketEntry entry) {
   ...
   if(entry == SHORT && expertIsTakingSellTrade) {
        ...
        tradeStopLoss = NormalizeDouble(dynamicPIP * stopLoss, _Digits);
        trade.SetExpertMagicNumber(EXPERT_MAGIC);
        Print("Current Price: ", bid, " Swing Price: ", swingPrice, " takeProfit: ", tradeTakeProfit, " StopLoss: ", tradeStopLoss);
...
int PTLHandle = iCustom(Symbol(), Period(), "SPTL2", inpFastLength, inpSlowLength);
bool PTLSELL = false;
bool PTLBUY = false;
