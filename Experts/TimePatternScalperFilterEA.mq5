...
input MARKET_TREND                                 Pattern_TREND = Bullish; // Market Trend
...
void OnTick()
{
   ...
   // Print("start: ", tradeStartTime, " Current:", tradeCurrentTime, " DIff:", tradeCurrentTime-tradeStartTime);
   ...
   if(timestamp != time) {
        ...
        if(StringCompare(search, pattern) == 0 && marketTrend(Pattern_TREND))
        {
            Print("Got A Match");
            if(PositionsTotal())
            {
