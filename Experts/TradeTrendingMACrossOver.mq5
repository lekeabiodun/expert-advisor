...
bool marketSignal(enumTradeType signal ){
   double last_tick[];
   ArraySetAsSeries(last_tick, true);
   CopyBuffer(FastMovingAverageHandle, 0, 1, 2, last_tick);

   if(signal == Buy){
      if(movingAverageCrossOver(Buy)) {
         if(closeOnOppositeSignal){ trade.PositionClose(PositionGetSymbol(0)); }
         if(marketTrend(Bullish)){
            ObjectCreate(0, IntegerToString(timestamp), OBJ_ARROW_UP, 0, TimeCurrent(), (last_tick[0]));
      ...
      if(movingAverageCrossOver(Sell)) {
         ...
         if(marketTrend(Bearish)){
            ObjectCreate(0, IntegerToString(timestamp), OBJ_ARROW_DOWN, 0, TimeCurrent(), (last_tick[0]));
