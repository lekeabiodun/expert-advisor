...
bool marketSignal(enumTradeType signal ){
   ...
   if(signal == Buy){
      if(movingAverageCrossOver(Buy)) {
         if(closeOnOppositeSignal){ trade.PositionClose(PositionGetSymbol(0)); }
         if(marketTrend(Bullish) && trixTrend(Bullish)){
            ObjectCreate(0, IntegerToString(timestamp), OBJ_ARROW_UP, 0, TimeCurrent(), (PriceInformation[1].close));
      ...
      if(movingAverageCrossOver(Sell)) {
         if(closeOnOppositeSignal){ trade.PositionClose(PositionGetSymbol(0)); }
         if(marketTrend(Bearish) && trixTrend(Bearish)){
            ObjectCreate(0, IntegerToString(timestamp), OBJ_ARROW_DOWN, 0, TimeCurrent(), (PriceInformation[1].open));
...
bool trixTrend(enumMarketTrend trend){
   ...
   if(trend == Bullish && ( trixArray[0] > 0.0 || trixArray[0] < trixArray[1]) ) { return true; }
   else if(trend == Bearish && (trixArray[0] < 0.0 || trixArray[0] > trixArray[1]) ) { return true; }
   return false;
}
