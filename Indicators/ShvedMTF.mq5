...
input ENUM_TIMEFRAMES SHVEDTF=PERIOD_CURRENT;// TimeFrame
...
int OnInit()
  {
   iATR_handle=iATR(NULL,SHVEDTF,7);
...
void CheckAlerts()
  {
   if(zone_show_alerts==false && zone_send_notification==false)
      return;
   datetime Time[];
   CopyTime(Symbol(),SHVEDTF,0,1,Time);
...
bool CheckEntryAlerts()
  {
   double Close[];
   ArraySetAsSeries(Close,true);
   CopyClose(Symbol(),SHVEDTF,0,1,Close);
// check for entries
      ...
      if(Close[0]>=zone_lo[i] && Close[0]<zone_hi[i])
        {
            ...
            if(zone_alert_popups==true)
              {
               if(zone_type[i]==ZONE_SUPPORT)
                  Alert(Symbol()+" "+TFTS(SHVEDTF)+": Support Zone Entered.");
               else
                  Alert(Symbol()+" "+TFTS(SHVEDTF)+": Resistance Zone Entered.");
              }
         ...
         if(zone_send_notification==true)
           {
            if(zone_type[i]==ZONE_SUPPORT)
               SendNotification(Symbol()+" "+TFTS(SHVEDTF)+": Support Zone Entered.");
            else
               SendNotification(Symbol()+" "+TFTS(SHVEDTF)+": Resistance Zone Entered.");
           }
...
void DeleteGlobalVars()
  {
   ...
   GlobalVariableDel("SSSR_Count_"+Symbol()+TFTS(SHVEDTF));
   GlobalVariableDel("SSSR_Updated_"+Symbol()+TFTS(SHVEDTF));
...
void DeleteOldGlobalVars(int old_count)
  {
   ...
   for(int i=zone_count; i<old_count; i++)
     {
      GlobalVariableDel("SSSR_HI_"+Symbol()+TFTS(SHVEDTF)+string(i));
      GlobalVariableDel("SSSR_LO_"+Symbol()+TFTS(SHVEDTF)+string(i));
      GlobalVariableDel("SSSR_HITS_"+Symbol()+TFTS(SHVEDTF)+string(i));
      GlobalVariableDel("SSSR_STRENGTH_"+Symbol()+TFTS(SHVEDTF)+string(i));
      GlobalVariableDel("SSSR_AGE_"+Symbol()+TFTS(SHVEDTF)+string(i));
     }
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
// finding those that have survived through to the present___
   shift=MathMin(Bars(Symbol(),SHVEDTF)-1,BackLimit+cnt);
   double Close[],High[],Low[];
   ArraySetAsSeries(Close,true);
   CopyClose(Symbol(),SHVEDTF,0,shift+1,Close);
   ArraySetAsSeries(High,true);
   CopyHigh(Symbol(),SHVEDTF,0,shift+1,High);
   ArraySetAsSeries(Low,true);
   CopyLow(Symbol(),SHVEDTF,0,shift+1,Low);
      ...
      if(temp_hits[i]>=0 && zone_count<1000)
        {
         ...
         if(zone_hi[zone_count]<Close[4])
            ...
            if(zone_lo[zone_count]>Close[4])
              ...
              {
               int  sh=MathMin(Bars(Symbol(),SHVEDTF)-1,BackLimit+cnt);
               for(j=5; j<sh; j++)
                 {
...
void DrawZones()
  {
   ...
   if(SetGlobals==true)
     {
      GlobalVariableSet("SSSR_Count_"+Symbol()+TFTS(SHVEDTF),zone_count);
      GlobalVariableSet("SSSR_Updated_"+Symbol()+TFTS(SHVEDTF),TimeCurrent());
     }
   ...
   for(int i=0; i<zone_count; i++)
     {
         ...
         if(zone_strength[i]==ZONE_VERIFIED)
            ...
            if(zone_strength[i]==ZONE_UNTESTED)
               ...
               if(zone_strength[i]==ZONE_TURNCOAT)
                  ...
                  s=s+"Weak";
      datetime Time[];
      CopyTime(Symbol(),SHVEDTF,0,zone_start[i]+1,Time);
      ArraySetAsSeries(Time,true);
      ...
      if(SetGlobals==true)
        {
         GlobalVariableSet("SSSR_HI_"+Symbol()+TFTS(SHVEDTF)+string(i),zone_hi[i]);
         GlobalVariableSet("SSSR_LO_"+Symbol()+TFTS(SHVEDTF)+string(i),zone_lo[i]);
         GlobalVariableSet("SSSR_HITS_"+Symbol()+TFTS(SHVEDTF)+string(i),zone_hits[i]);
         GlobalVariableSet("SSSR_STRENGTH_"+Symbol()+TFTS(SHVEDTF)+string(i),zone_strength[i]);
         GlobalVariableSet("SSSR_AGE_"+Symbol()+TFTS(SHVEDTF)+string(i),zone_start[i]);
        }
...
bool Fractal(int M,int P,int shift)
  {
   ...
   P=int(P/SHVEDTF*2+MathCeil(P/SHVEDTF/2));

   if(shift<P)
      return(false);

   if(shift>Bars(Symbol(),SHVEDTF)-P-1)
      return(false);
   double High[],Low[];
   ArraySetAsSeries(High,true);
   CopyHigh(Symbol(),SHVEDTF,0,shift+P+1,High);
   ArraySetAsSeries(Low,true);
   CopyLow(Symbol(),SHVEDTF,0,shift+P+1,Low);
   for(int i=1; i<=P; i++)
     {
      if(M==UP_POINT)
        {
...
//--- FastFractals
   int shift;
   int limit=MathMin(Bars(Symbol(),SHVEDTF)-1,BackLimit+cnt);
   int P1=int(SHVEDTF*fractal_fast_factor);
   double High[],Low[];
   ArraySetAsSeries(High,true);
   CopyHigh(Symbol(),SHVEDTF,0,limit+1,High);
   ArraySetAsSeries(Low,true);
   CopyLow(Symbol(),SHVEDTF,0,limit+1,Low);
   FastUpPts[0] = 0.0;
   FastUpPts[1] = 0.0;
   FastDnPts[0] = 0.0;
   FastDnPts[1] = 0.0;
...
//--- SlowFractals
   int shift;
   int limit=MathMin(Bars(Symbol(),SHVEDTF)-1,BackLimit+cnt);
   int P2=int(SHVEDTF*fractal_slow_factor);
   double High[],Low[];
   ArraySetAsSeries(High,true);
   CopyHigh(Symbol(),SHVEDTF,0,limit+1,High);
   ArraySetAsSeries(Low,true);
         ...
         if(clickTime < clickTimeMemory + 500)
           {
            ...
            if(ChartXYToTimePrice(0,x,y,window,dt,price))
              {
               cnt=iBarShift(_Symbol,SHVEDTF,dt,false);
               int old_zone_count=zone_count;
               FastFractals();
               SlowFractals();
               DeleteZones();
...
void showLabels()
  {
   datetime Time[];
   CopyTime(Symbol(),SHVEDTF,cnt,1,Time);
   ArraySetAsSeries(Time,true);
   for(int i=0; i<zone_count; i++)
     {
      string lbl;
